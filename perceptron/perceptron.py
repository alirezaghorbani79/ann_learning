...
x, y = datasets.make_blobs(n_samples=200, centers=[(-1, -1), (1, 1)], cluster_std=0.5, shuffle=True)
...
for ep in range(epochs):
    ...
    for inputs, label in zip(x, y):
        ...
        if (label != prediction):
            ...
            plt.xlim(-3, 3)
            plt.ylim(-3, 3)
            plt.text(0, -2.7, 'epoch|iter = {:2d}|{:2d}'.format(ep, i), fontdict={'size': 14, 'color':  'blue'})
            plt.pause(0.01)
